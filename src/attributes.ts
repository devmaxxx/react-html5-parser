...
export const attributesMap = attrs.split(" ").reduce<AtributesMap>(
  (acc, prop) => {
    ...
    if (isNoCamelCase) acc[value.toLowerCase()] = value;
...
export function attrsToProps(attrs: NamedNodeMap): Attributes {
  return Array.from(attrs).reduce<Attributes>(
    (acc, { nodeName, nodeValue }) => {
      const attrKey =
        attributesMap[nodeName] || (nodeName as AtributesPropKeys);
